    ...
    return (
      <>
        ...
        <Row style={{minHeight:'50px'}} gutter={10}>
          ...
          <Col>
            <span className='myFontColor'>{t('time')}</span><SwitchablePicker setState={props.setState} month={props.month} type={props.type} year={props.year}/>
          </Col>
          <Col>
          {
            props.hasDetail==true?
            <><span className='myFontColor'>{t('detail')}</span> <Switch defaultChecked={props.showDetail} onChange={(checked)=>{props.setState({'showDetail':checked})}} /></>
            :<></>
          }
          </Col>
        </Row>
